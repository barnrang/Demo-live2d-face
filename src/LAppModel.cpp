...
void LAppModel::Update(cv::VideoCapture& cap)
{
    ...
    //ドラッグによる変化
    //ドラッグによる顔の向きの調整
    //_model->AddParameterValue(_idParamAngleX, _dragX * 50); // -30から30の値を加える
    //_model->AddParameterValue(_idParamAngleY, _dragY * 50);
    //_model->AddParameterValue(_idParamAngleZ, _dragX * _dragY * -50);

	float w = 0.4;

	cv::Vec3d tmp = _cameraManager.GetHeadRotation(cap);
	if (tmp[0] != 999 && tmp[2] > 0) {
		prevAngle = w * tmp + (1 - w) * prevAngle;

		prev_xrotate = - 2.5f / 3.f * ((prevAngle[0] - 120) + (prevAngle[2] - 135));
		prev_yrotate = 2.5f * (-5.f / 6.f * (prevAngle[0] - 120) + 2.f / 3.f * (prevAngle[2] - 135));
		prev_zrotate = 30 - prevAngle[1];

		LAppPal::PrintLog("x=%f y=%f\n", prev_xrotate, prev_yrotate);


		//_model->AddParameterValue(_idParamAngleX, 130 - prevAngle[2]); // -30から30の値を加える
		//_model->AddParameterValue(_idParamAngleY, (120 - prevAngle[0]) * 2.f);

		_model->AddParameterValue(_idParamAngleX, prev_xrotate); // -30から30の値を加える
		_model->AddParameterValue(_idParamAngleY, prev_yrotate);
		_model->AddParameterValue(_idParamAngleZ, prev_zrotate);
		_model->AddParameterValue(_idParamEyeBallX, prev_xrotate / 30.f); // -1から1の値を加える
		_model->AddParameterValue(_idParamEyeBallY, prev_yrotate / 30.f);
		//prevAngle = tmp;
		float tmp_mouth = _cameraManager.GetMouthSize();
		prevMouth = w * tmp_mouth + (1 - w) * prevMouth;
		_model->AddParameterValue(_idParamMouthOpenY, prevMouth);

		float l_eye_ratio = _cameraManager.GetLEyeSize();
		float r_eye_ratio = _cameraManager.GetREyeSize();
		if (l_eye_ratio < 0.1) {
			_model->AddParameterValue(_idParamEyeLOpen, -1);
		}	
		else {
			_model->AddParameterValue(_idParamEyeLOpen, 1);
		}
		...
		if (r_eye_ratio < 0.1) {
			_model->AddParameterValue(_idParamEyeROpen, -1);
		}
		else {
			_model->AddParameterValue(_idParamEyeROpen, 1);
		}
		//_cameraManager.GetREyeSize();
	}
	else {
		_model->AddParameterValue(_idParamAngleX, prev_xrotate);
		_model->AddParameterValue(_idParamAngleY, prev_yrotate);
		_model->AddParameterValue(_idParamAngleZ, prev_zrotate);
		_model->AddParameterValue(_idParamEyeBallX, prev_xrotate / 40.f); // -1から1の値を加える
		_model->AddParameterValue(_idParamEyeBallY, prev_yrotate / 40.f);
	...
	//_model->AddParameterValue(_idParamMouthOpenY, 0.);
...
CubismMotionQueueEntryHandle LAppModel::StartRandomMotion(const csmChar* group, csmInt32 priority, ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    csmInt32 no = std::rand() % _modelSetting->GetMotionCount(group);
...
void LAppModel::SetRandomExpression()
{
    ...
    csmInt32 no = std::rand() % _expressions.GetSize();
