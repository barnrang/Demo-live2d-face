...
public:
     ...
     *
     */
    void Update(cv::VideoCapture& cap);
...
private:
    ...
    */
	...
	cv::Mat inv = 2.f * (cv::Mat_<double>(2, 2) << 1.f / 3.f, 1.f / 3.f, -5.f / 6.f, 2.f / 3.f);

	cv::Vec3d prevAngle;
	float prevMouth=0.;
	float prev_xrotate = 0.;
	float prev_yrotate = 0.;
	float prev_zrotate = 0.;

	LCameraManager _cameraManager;
